...
namespace Ext.Algorithms.Implementation.BTree
{
    ...
    public class BTree<TK, TP> where TK : IComparable<TK>
    {
        public BTree(int degree)
        {
            ...
            Root = new BTreeNode<TK, TP>(degree);
            Degree = degree;
            Height = 1;
        }
        ...
        public void BulkInsert(IEnumerable<KeyValuePair<TK, TP>> values)
        {
        ...
        private static Entry<TK, TP> DeletePredecessor(BTreeNode<TK, TP> node)
        {
            if (!node.IsLeaf) return DeletePredecessor(node.Children.Last());
        ...
        private static Entry<TK, TP> DeleteSuccessor(BTreeNode<TK, TP> node)
        {
            if (!node.IsLeaf) return DeletePredecessor(node.Children.First());
        ...
        private Entry<TK, TP> SearchInternal(BTreeNode<TK, TP> node, TK key)
        {
        ...
        {
            var newNode = new BTreeNode<TK, TP>(Degree);
